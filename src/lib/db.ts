...
export function db() {
  ...
  connection.exec(`
    ...
    CREATE TABLE IF NOT EXISTS admission_rate_limits (key TEXT PRIMARY KEY, count INTEGER NOT NULL, resets_at INTEGER NOT NULL);
  ...
  if (schemaVersion < 52) {
    connection.transaction(() => {
      const columns = connection.pragma('table_info(schools)') as { name: string }[];
      if (!columns.some((column) => column.name === 'teacher_checkin_late_enabled'))
        connection.exec(
