...
const bodySchema = z
  .object({ records: z.array(recordSchema).min(1).max(1000) })
  ...
  try {
    ...
    db().transaction(() => {
      const update = db().prepare(
        `UPDATE extracurricular_attendance_records
         SET status=?,note=?,source='native_app',updated_by=?,updated_at=datetime('now')
      ...
      audit(actor.email, 'update', 'extracurricular_attendance_records', sessionId, {
        source: 'mobile',
